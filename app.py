...
if st.button(label= 'Show Recommendations'):
    ...
    with col1:
        st.text(reccommended_movie_name[0])
        st.image(recommended_movie_poster[0])
    with col2:
        st.text(reccommended_movie_name[1])
        st.image(recommended_movie_poster[1])

    with col3:
        st.text(reccommended_movie_name[2])
        st.image(recommended_movie_poster[2])
    with col4:
        st.text(reccommended_movie_name[3])
        st.image(recommended_movie_poster[3])
    with col5:
        st.text(reccommended_movie_name[4])
